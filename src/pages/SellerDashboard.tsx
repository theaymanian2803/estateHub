...
import SellerInquiries from '@/components/SellerInquiries' // IMPORT THE NEW COMPONENT
import { useTranslation } from 'react-i18next'

// Add "messages" to the Tab types
type Tab = 'overview' | 'create' | 'manage' | 'edit' | 'subscription' | 'profile' | 'messages'
...
export default function SellerDashboard() {
  ...
  // NEW: Track unread messages for the overview dashboard
  ...
  const refreshListings = async () => {
    ...
    // NEW: Fetch unread messages count for this seller
  ...
  // Added Messages to the visible tabs
  const visibleTabs: { id: Tab; label: string; icon: React.ElementType; badge?: number }[] = [
  ...
  const handleCreate = async (formData: PropertyFormData) => {
    if (atLimit) {
      toast({
        title: t('dashboard.listingLimitReached'),
        description: `${t('dashboard.upgradeDesc')}`,
        variant: 'destructive',
      })
      return
    }
    ...
    const { error } = await supabase
      .from('properties')
      .insert({ user_id: user.id, ...formData, status: isAdmin ? 'active' : 'pending' })
    if (error) {
      ...
      toast({
        title: isAdmin ? t('dashboard.listingPublished') : t('dashboard.listingSubmitted'),
        description: isAdmin ? t('dashboard.propertyLive') : t('dashboard.pendingApproval'),
      })
  ...
  const handleUpdate = async (formData: PropertyFormData) => {
    ...
    if (error) {
      ...
      toast({ title: t('dashboard.listingUpdated'), description: t('dashboard.changesSaved') })
      setEditingId(null)
      setEditData(null)
      await refreshListings()
      setTab('manage')
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    ...
    if (error) {
      ...
      toast({
        title: t('dashboard.statusUpdated') || 'Status updated',
  ...
  return (
    <div className="min-h-screen bg-secondary/30">
      ...
      <div className="container mx-auto px-4 pb-16 pt-24">
        ...
        <div className="mt-6 flex flex-wrap gap-2 border-b border-border pb-2">
          ...
          {tab === 'edit' && (
            <span className="flex items-center gap-1.5 rounded-md bg-accent px-4 py-2 text-sm font-medium text-accent-foreground">
              <Edit className="h-4 w-4" /> {t('dashboard.editListing')}
            </span>
          )}
        </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-foreground">
                  {currentPlan === 'free' ? t('dashboard.freePlan') : t('dashboard.sellerPro')} —{' '}
                  {myListings.length}/{listingLimit} {t('dashboard.listingsUsed')}
                </p>
                <p className="text-xs text-muted-foreground">
                  {currentPlan === 'free'
                    ? t('dashboard.upgradeDesc')
                    : t('dashboard.upgradeUnlimitedDesc')}
                </p>
              </div>
            </div>
            <Button
              ...
              asChild>
              <Link to="/pricing">
                {t('dashboard.upgrade')} <ArrowRight className="ml-1 h-3 w-3" />
              </Link>
            </Button>
          </div>
        )}
        ...
        {showWelcomeBanner && (
          <div className="mt-6 relative flex flex-col gap-3 rounded-lg border border-primary/30 bg-primary/5 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-foreground">{t('dashboard.welcomePro')}</p>
                <p className="text-xs text-muted-foreground">{t('dashboard.welcomeProDesc')}</p>
              </div>
            </div>
            <Button
              size="sm"
              ...
              {t('dashboard.createAListing')} <ArrowRight className="ml-1 h-3 w-3" />
            </Button>
            <button
            ...
            <div className="mt-8 grid gap-6 sm:grid-cols-4">
              {[
                { label: t('dashboard.activeListings'), value: myListings.length, icon: List },
                {
                  ...
                  value: myListings.reduce((s, p) => s + (p.views || 0), 0).toLocaleString(),
                  icon: Eye,
                },
                {
                  label: 'Total Saves',
                  value: savesData.reduce((s, d) => s + d.saves_count, 0).toLocaleString(),
                  icon: Heart,
                },
                // NEW: Show real Unread Messages count
                {
        ...
        {/* NEW: Render the messages component */}
        {tab === 'messages' && <SellerInquiries />}

        {tab === 'create' && (
          <PropertyForm
            userId={user.id}
            onSubmit={handleCreate}
            submitLabel={t('dashboard.publishListing')}
          />
        )}
          ...
          <div className="mt-8 overflow-x-auto rounded-lg border border-border bg-card">
            {myListings.length === 0 ? (
              <div className="p-8 text-center text-muted-foreground">
                ...
                <Button variant="link" className="text-accent" onClick={() => setTab('create')}>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>{t('dashboard.property')}</TableHead>
                  ...
                  {myListings.map((p) => (
                    <TableRow key={p.id}>
                      ...
                      <TableCell>
                        ...
                        {!['active', 'pending', 'rejected', 'sold', 'under_contract'].includes(
                          p.status
                            ...
                            <DropdownMenuContent align="end">
                              {p.status !== 'active' && (
                                <DropdownMenuItem
                                  ...
                                  <CheckCircle className="mr-2 h-4 w-4 text-accent" /> Mark Active
                                </DropdownMenuItem>
                              )}
                              {p.status !== 'under_contract' && (
                                <DropdownMenuItem
                                  onClick={() => handleStatusChange(p.id, 'under_contract')}>
                                  <FileText className="mr-2 h-4 w-4 text-blue-600" /> Under Contract
                                </DropdownMenuItem>
                              )}
                              {p.status !== 'sold' && (
                                <DropdownMenuItem onClick={() => handleStatusChange(p.id, 'sold')}>
                                  <CheckCircle className="mr-2 h-4 w-4 text-primary" /> Mark Sold
                                </DropdownMenuItem>
                              )}
                            </DropdownMenuContent>
                          </DropdownMenu>
