...
// IMPORT THE NEW GLOBAL TRIGGER
import {
  useSiteContent,
  SiteContentItem,
  Testimonial,
  triggerGlobalSiteContentUpdate,
...
export default function AdminLandingControls() {
  ...
  const handleSaveAll = async () => {
    ...
    if (hasError) {
      ...
      setEditedValues({})
      triggerGlobalSiteContentUpdate() // Sync app instantly
    }
    setSaving(false)
  }
  ...
  const handleAddTestimonial = async () => {
    ...
    const avatar =
      ...
      newTestimonial.name
        ...
        .toUpperCase()
    const nextOrder =
      testimonials.length > 0 ? Math.max(...testimonials.map((t) => t.sort_order)) + 1 : 1
  ...
  const renderContentFields = (section: string) => {
    ...
    if (items.length === 0)
      return (
        <p className="text-sm text-muted-foreground italic">No content fields for this section</p>
      )
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Edit all text content and testimonials displayed on the homepage.
            </p>
          </div>
          {subTab !== 'testimonials' && (
            <div className="flex gap-2">
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      You have {Object.keys(editedValues).length} unsaved change(s). This action
                      cannot be undone.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
              ...
              <Button
                ...
                {saving
                  ? 'Saving…'
                  : `Save Changes${Object.keys(editedValues).length > 0 ? ` (${Object.keys(editedValues).length})` : ''}`}
              </Button>
            </div>
          )}
        </div>
                  ...
                  <div className="mb-4 flex h-20 items-center justify-center rounded-lg border-2 border-dashed border-border bg-muted/30">
                    <div className="text-center">
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        No logo set (using default icon)
                      </p>
                    </div>
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <Input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') handleSetLogoUrl()
                        }}
                        className="text-sm"
                      />
                      <Button
                ...
                {getValue('hero_bg_image') ? (
                  ...
                  <div className="mb-4 flex h-32 items-center justify-center rounded-lg border-2 border-dashed border-border bg-muted/30">
                    <div className="text-center">
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        No background image set (using default)
                      </p>
                    </div>
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <Input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') handleSetExternalUrl()
                        }}
                        className="text-sm"
                      />
                      <Button
        ...
        {subTab === 'stats' && (
          <div className="max-w-2xl">
            ...
            <p className="text-xs text-muted-foreground mb-4">
              Edit the numbers and labels shown in the stats bar below the hero.
            </p>
          ...
          <div>
            ...
            <div className="rounded-lg border border-border bg-secondary/30 p-4 mb-6 max-w-2xl">
              ...
              <div className="mt-3 flex items-center gap-4">
                ...
                <Input
                  ...
                  onChange={(e) => setNewTestimonial((p) => ({ ...p, avatar: e.target.value }))}
                  className="w-32"
                />
            ...
            <div className="space-y-3">
              {testimonials.map((t) => (
                <div
                  key={t.id}
                  className={`rounded-lg border p-4 flex items-start gap-4 transition-colors ${t.is_active ? 'border-border bg-card' : 'border-border/50 bg-muted/30 opacity-60'}`}>
